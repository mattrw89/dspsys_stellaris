...
fs_open(char *name)
{
	...
	data = name;
	Json temp;
	uint8_t match = http_process_url(data, strlen(data), &temp);
	if (match != 0) {
		//
        // Setup the file structure to return the json data
        //
        ptFile->data = (char *) &(temp.string[0]);
        ptFile->len = (temp.total_length+1)/2;
        ptFile->index = 0;
        ptFile->pextension = 0;
